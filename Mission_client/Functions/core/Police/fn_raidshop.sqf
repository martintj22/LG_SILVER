...
if (!(isNil "_player")) exitWith {
	...
	[myshop, format["%1's Shop",name player], "Job"] remoteexec ["client_fnc_hudHelper", _player];

	[[player, _player, myshop], {
		_suspect = _this select 0;
		_officer = _this select 1;
		_myshop = _this select 2;

		waitUntil {uiSleep 1; getPos _officer distance _myshop < 20}; 
		[_suspect, _officer] remoteExec ["server_fnc_retreiveStore",2]; 
...
if (_warrant > -1) exitWith {
	...
	[player] remoteexec ["client_fnc_raidshop", _suspect3];
