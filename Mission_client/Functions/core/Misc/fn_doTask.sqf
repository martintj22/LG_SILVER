
player say "slideout";
disableSerialization;
if(isnil "RunningTASK") then { RunningTASK = false; };
if(isnil "client_dotask") then { client_dotask = true; };
if(RunningTASK) exitwith {};
RunningTASK = true;
private["_remote"];
params["_task","_timeLeft","_process","_distanceCheck","_animation","_actions","_sound",["_remote",player]];

_success = true;
_error = "Success";

_factor = 1;


_level_default = (client_level_array select 0);
if(_level_default > 0) then { _factor = _level_default / 400; };


_timeLeft = _timeLeft - _factor;

if(_timeLeft < 1) then { _timeLeft = 3; };
_timeLeft = round ( _timeLeft );

21 cutRsc ["RSC_dotask","PLAIN"];
_POPTASK = uiNameSpace getVariable ["RSC_dotask",displayNull];
_POPUP = _POPTASK displayCtrl 9119;
_POPUP ctrlSetStructuredText parseText format["<img size='1' image='cg_mission_files\icons\info.paa'/> <t color='#FFCC00'><t size='0.9'>%1</t> <br/> <t size='2'>%2</t>",_task,_timeLeft];

if !(_sound isEqualTo 0) then { 
	playSound3D [_sound, player, false, getPosASL player, 8, 1, 45];
};

if (_distanceCheck isEqualTo 0) then { 
	_distanceCheck = player;
};

if (typeName _distanceCheck == "OBJECT") then {
	_distanceCheck = getPosATL _distanceCheck;
};

while{true} do {

	uisleep 1;
	_timeLeft = _timeLeft - 1;
	_POPUP ctrlSetStructuredText parseText format["<img size='1' image='cg_mission_files\icons\info.paa'/> <t color='#FFCC00'><t size='0.9'>%1</t> <br/> <t size='2'>%2</t>",_task,_timeLeft];
	if(_timeLeft == 0) exitwith {};

	if( !(_animation isEqualTo 0) ) then {
		if((animationstate player) != _animation) then { player playmove _animation; };
	};
	if((getposATL player) distance _distanceCheck > 5) exitwith { _success = false; _error = "Movement Error"; };
	if(!_success) exitwith {};		
	if(ClientInterrupted) exitwith { _success = false; _error = "Client Interrupted"; player switchmove ""; };
	if(DeadPlayer) exitwith { _success = false; _error = "Dead Player"; };	

};

if !(_animation isEqualTo 0) then { 
	player switchMove "";
};
	
_POPUP ctrlSetStructuredText parseText format["<img size='1' image='cg_mission_files\icons\info.paa'/> <t color='#FFCC00'><t size='0.9'>%1</t> <br/> <t size='1'>%2</t>",_task,_error];
player say "slideout";
_POPUP ctrlSetStructuredText parseText format["",_task,_timeLeft];
RunningTASK = false;
if(!_success) exitwith { client_dotask = false; }; 

if !(_process isEqualTo 0) then 
{ 
	if(_actions isEqualTo 0) then 
	{ 
		if(_remote == player) exitwith { 
			_script = ["[] spawn", _process] joinstring " "; 
			call compile _script;
		};
		[] remoteExec [_process, _remote]; 
	} else { 
		if(_remote == player) exitwith { 
			_script = ["_actions spawn", _process] joinstring " "; 
			call compile _script;
		 };
		_actions remoteExec [_process, _remote]; 
	}; 
	
};

client_dotask = true;