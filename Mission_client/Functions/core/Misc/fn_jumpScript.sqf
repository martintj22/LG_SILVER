...
if (!(EM_allowed2) || (typeof cursorobject) IN nonclimbables) exitwith {};
	...
	{ 
		...
		if (_intcount == 0) then
		{
			...
			if (_anm != "" && str _dpos != "[0,0,0]") then
			{
				...
				["EH_em_drop", "animationState (_condpars select 0) == (_condpars select 1)", [_climber, _anm], "babe_em_fnc_exec_drop", [_dpos, _climber], true, "babe_em_fnc_finish_drop", [_climber], -1] call babe_core_fnc_addEH;
