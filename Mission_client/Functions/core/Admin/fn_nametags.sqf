...
{
	private["_text"];
	_pos = [visiblePosition _x select 0, visiblePosition _x select 1, ((_x modelToWorld (_x selectionPosition "head")) select 2)+.25];
	_sPos = worldToScreen _pos;
	_distance = _x distance player;
	//_name = _x getVariable ["realname", name _x];
	_name = [getplayeruid _x, 7] call CBA_fnc_substr;
	_dickbag = true;
	_text = "";
	if( count _sPos > 1 && isPlayer _x && _distance < 8 && _x != player && vehicle player == player && currentweapon player == "" && !(_x getVariable["admingod",FALSE]) && (_x getVariable["talking",FALSE]) ) then {
