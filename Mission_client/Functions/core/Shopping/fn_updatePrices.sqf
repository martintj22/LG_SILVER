...
if(_toDo == "nameUpdate") exitwith {
	["Shop name is being processed.", true] spawn domsg;
...
while{_currentShop < 4} do {
	...
	while { _n != _loop } do {
		...
		_list lbAdd format["%1 | Qty: %2 | Price: %3", _item select 1, ((shopcontent select _currentShop) select 1) select _n, (((shopcontent select _currentShop) select 2) select _n) call client_fnc_numberText];
