...
stress_effects = {
  ...
  1 setFog 1;
  addCamShake [random 0.2,random 0.2,random 0.2];
  uisleep 1;
  1 setFog 0;
...
drug_heroin_effects = { _this spawn {
  ...
  waitUntil {
    ...
    if ((random 100) < 5) then {
      waituntil {
        addCamShake [random 0.5,random 0.5,random 0.5];
...
drug_beer_effects = { _this spawn {
  ...
  _effect2 = ppEffectCreate ["chromAberration",70];
  _effect2 ppEffectEnable true;
