		...
		{ 
			_itemNumber = _ItemCount select _count;
			 if !(_x IN _oreArray) then {
				...
				if(str _x find "5" > -1) then { _itemnumber = _itemnumber * 5;  _holder addItemCargoGlobal [(_barArray select 0),_itemNumber]; };
				if(str _x find "6" > -1) then { _itemnumber = _itemnumber * 6;  _holder addItemCargoGlobal [(_barArray select 0),_itemNumber]; };
				_count = _count + 1;
				playSound3D ["vvv_fishingrod\sounds\splash.ogg", player, false, getPosasl player, 5, 1, 5];
				if(dialog) then { closedialog 0; };
				uisleep 0.25;
